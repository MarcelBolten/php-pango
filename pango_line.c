...
static void php_pango_layoutline_write_property(
    ...
    if ((Z_STRLEN_P(member) == sizeof("length") - 1
         && !memcmp(Z_STRVAL_P(member), "length", sizeof("length")))
        || (Z_STRLEN_P(member) == sizeof("is_paragraph_start") - 1
            && !memcmp(Z_STRVAL_P(member), "is_paragraph_start", sizeof("is_paragraph_start")))
        || (Z_STRLEN_P(member) == sizeof("resolved_dir") - 1
            && !memcmp(Z_STRVAL_P(member), "resolved_dir", sizeof("resolved_dir")))
    ) {
